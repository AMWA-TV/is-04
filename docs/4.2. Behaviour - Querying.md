...
The Query API provides a read-only interface to the Nodes and their sub-resources which are currently held in the registry. Browsing of the Query API can be performed using the HTTP API, or using a WebSocket subscription ([RFC 6455](https://tools.ietf.org/html/rfc6455)), which is suggested for most applications.
