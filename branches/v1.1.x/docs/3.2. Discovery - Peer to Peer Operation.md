...
The following pre-requisites are mandatory aspects of the [Node API](../html-html-APIs/NodeAPI.html) implementation which must be in place to permit peer to peer discovery.
...
5. The Node continues to monitor for changes to Node advertisements via mDNS. When Node API resources are changed, the TXT records associated with each Node will be updated to indicate which API resource(s) have been updated (see [Node API specification](../html-html-APIs/NodeAPI.html))
